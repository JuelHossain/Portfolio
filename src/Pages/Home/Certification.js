import { Avatar, Container, Flex, Stack } from "@chakra-ui/react";
  ...
  const certificateCards = certificates.map((certificate) => {
    ...
    return (
      <Stack
        ...
        as={"a"}
        href={link}
        className="dark:border-gray-700  flex-1 border shadow-md dark:hover:bg-red-900/20 hover:bg-gray-200 duration-300 rounded-md basis-60  dark:bg-gray-800 p-5 gap-1"
      >
        <p className="text-xl bg-yellow-500/20 dark:bg-red-500/20 font-bold capitalize px-2 py-1  ">{name}</p>
        <Flex gap={2}>
